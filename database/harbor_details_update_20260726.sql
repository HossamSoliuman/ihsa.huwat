-- Harbor details module update
-- Import this file once after selecting the project's existing database
-- (the current local configuration names it `ihsa`).
...
-- Legacy rows cannot be classified accurately from the old schema. Mark them
-- unclassified so they are excluded from harbor occupancy statistics until an
-- authorized user reviews them in Harbor Details > Manage Boats.
ALTER TABLE boats
...
-- This also upgrades databases on which an earlier draft added narrower ENUMs.
-- Existing non-default classifications are deliberately preserved.
ALTER TABLE boats
...
-- No example capacities are inserted. Each harbor remains at zero until its
-- real large/small/recreational capacities are entered by an authorized user.
