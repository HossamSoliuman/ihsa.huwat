...
class StoreInformationSubmissionRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $crewMembers = collect((array) $this->input('crew_members', []))
            ->map(function (mixed $crewMember): array {
                ...
                return [
                    ...
                    'passport_number' => $this->normalizeCode(Arr::get($crewMember, 'passport_number')),
                    'experience_years' => $this->normalizeDigits(Arr::get($crewMember, 'experience_years')),
                ];
            })
            ->values()
            ->all();
        ...
        $this->merge([
            ...
            'captain_passport_number' => $this->normalizeCode($this->input('captain_passport_number')),
            'captain_license_number' => $this->normalizeCode($this->input('captain_license_number')),
            'captain_experience_years' => $this->normalizeDigits($this->input('captain_experience_years')),
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'captain_passport_number' => ['required', 'regex:/^[A-Z0-9\-\/]{5,30}$/'],
            'captain_birth_date' => ['required', 'date_format:Y-m-d', 'before_or_equal:today'],
            ...
            'captain_qualification' => ['required', Rule::in(array_keys(config('information.captain_qualifications')))],
            'captain_experience_years' => ['required', 'integer', 'between:0,60'],
            ...
            'crew_members.*' => ['array:full_name,identity_number,passport_number,phone,birth_date,nationality,role,experience_years'],
            'crew_members.*.full_name' => ['required', 'string', 'min:3', 'max:150'],
            'crew_members.*.identity_number' => ['required', 'regex:/^[12][0-9]{9}$/', 'distinct'],
            'crew_members.*.passport_number' => ['nullable', 'regex:/^[A-Z0-9\-\/]{5,30}$/', 'distinct'],
            'crew_members.*.phone' => ['required', 'regex:/^\+?[0-9]{8,15}$/'],
            'crew_members.*.birth_date' => ['required', 'date_format:Y-m-d', 'before_or_equal:today'],
            ...
            'crew_members.*.experience_years' => ['required', 'integer', 'between:0,60'],
    ...
    public function messages(): array
    {
        return [
            ...
            'crew_members.*.passport_number.distinct' => 'رقم جواز كل بحار يجب أن يكون فريداً.',
