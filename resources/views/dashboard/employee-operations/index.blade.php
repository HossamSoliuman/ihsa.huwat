...
<div class="operations-deck">
    ...
    @else
        <header class="operations-command"><div><span>LIVE COUNTING STATION</span><h1>{{ $assignment->port->name }}</h1><p>{{ $assignment->shift->name }} · <span dir="ltr">{{ $assignment->shift->start_time }}—{{ $assignment->shift->end_time }}</span></p></div><dl><div><dt>الموظف</dt><dd>{{ auth()->user()->full_name }}</dd></div><div><dt>تاريخ التشغيل</dt><dd dir="ltr">{{ today()->format('Y/m/d') }}</dd></div></dl></header>
