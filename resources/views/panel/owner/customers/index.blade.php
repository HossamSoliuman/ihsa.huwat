...
@section('content')
    <div class="page-header">
        <div class="lead">
            ...
            <div>
                <h1>العملاء</h1>
                <p>من تبيع لهم المصيد — القائمة نفسها التي يبحث فيها التطبيق عند تسجيل بيع</p>
            </div>
        </div>
        <div class="actions">
    ...
    <form method="GET" class="filter-bar" style="margin-bottom:1.25rem">
        ...
        <a href="{{ route('panel.owner.customers') }}" class="btn btn-outline">إعادة تعيين</a>
    </form>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;gap:.25rem;justify-content:flex-end">
                                ...
                                <form method="POST" action="{{ route('panel.owner.customers.destroy', $row) }}" onsubmit="return confirm('حذف {{ $row->name }}؟')">
                                    @csrf @method('DELETE')
    ...
    <div class="drawer" id="recordDrawer">
        ...
        <form method="POST" id="recordFormEl" action="{{ route('panel.owner.customers.store') }}" class="drawer-body" autocomplete="off">
            @csrf
...
<script>
    const recordForm = { drawer: 'recordDrawer', form: 'recordFormEl', title: 'recordFormTitle', storeUrl: @json(route('panel.owner.customers.store')), createTitle: 'عميل جديد', editTitle: 'تعديل العميل' };
